...
def run_one(cap: str, band: str, close: str, size: str, seed: int) -> dict:
...
def main() -> None:
    todo = [(cap, band, close, size, seed)
            for (cap, band, close, size) in ARMS for seed in SEEDS]
    print(f"scan: {len(ARMS)} arms x {len(SEEDS)} seeds = {len(todo)} runs "
          f"at n={N}, T={T:,}")
    t0 = time.time()
    with open(OUT, "w") as f:
        for i, (cap, band, close, size, seed) in enumerate(todo, 1):
            row = run_one(cap, band, close, size, seed)
